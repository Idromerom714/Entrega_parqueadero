...
ifeq ($(PLATFORM),Windows)
    CLIENTE := $(CLIENTE).exe
    # Obtener ruta de librerías de Python
    PYTHON_LIBDIR := $(shell $(PYTHON) -c "import sysconfig; print(sysconfig.get_config_var('LIBDIR') or sysconfig.get_path('stdlib'))")
    PYTHON_VERSION := $(shell $(PYTHON) -c "import sys; print(f'python{sys.version_info.major}{sys.version_info.minor}')")
    PYTHON_LIBS := -L$(PYTHON_LIBDIR) -l$(PYTHON_VERSION)
endif
...
ifeq ($(PLATFORM),Windows)
	$(CXX) $(CXXFLAGS) $(INCLUDES) $(SOURCES) -o $(MODULE) $(SOCKET_LIBS) $(PYTHON_LIBS)
else
